...
function ai_bp_reactions_settings_check_existance($table, $condition, $value)
{
	...
	{
		$wpdb->update($table,array( 'value' => $value), array( 'type' =>$condition ), array( '%s'), array( '%s' ) );
	}
}
...
add_action('wp_enqueue_scripts', 'ai_bp_main_animation_css');
...
manage smileys 
*************/
class TT_Example_List_Table_smiley extends WP_List_Table
{
    public $main_data;
    public $type;
    /** ************************************************************************
...
function ai_bp_reactions_html()
{
    ...
    $html .= '<div class="ai_bp_reactions_counter"><a href="#" class="expand ai_emo_button" id="ai_counter" original-title="' . esc_attr($selected_name) .'" ><span class="ai_emo_counter">' . $count . '</span></a></div>';
...
function ai_other_user_smiley_activity($check_users, $smiley_id, $userid)
{
    ...
    foreach ($result as $val) {
        ...
        $image_plugin = esc_url(plugins_url('activity-reactions-for-buddypress/img/') . $image);
        $html .= '<a href smiley_id="' . $id . '" class="ai_bp_reactions_acted ai_emo_button ai_other_popup expand" original-title="' . esc_attr($name_smiley) .'"><img src="' . esc_url($image_plugin) . '" class="smiley_img" smiley_id="' . $id . '" "></a>';
    }
...
function ai_get_smiley()
{
    ...
    foreach ($result as $val) {
			...
			if($check == 'checked')
			{
				$html .= '<li><a href smiley_id="' . $id . '" user_id="' . $user_id . '" class="ai_bp_reactions expand" original-title="' . esc_attr($name_smiley) . '-"><img src="' . esc_url($image_plugin) . '" class="smiley_img" smiley_id="' . $id . '"></a></li>';
			}
            $count++;
    }
    $html .= '</ul></div>';
    ...
    foreach ($result as $val) {
		...
		if($check == 'checked')
		{
			if (check_ai_bp_reactions_default($id, $activity_id, $user_id) == "exist") {
				...
				$html .= '<li><a href smiley_id="' . $id . '" user_id="' . $user_id . '" class="ai_bp_reactions expand" original-title="' . esc_attr($name_smiley) . '"><img src="' . esc_url($image_plugin) . '" class="smiley_img ai_'.$count.'" smiley_id="' . $id . '"></a></li>';
			}
			$main_count++;
		}
		$count++;
...
function ai_get_selected_smiley($id, $activity_id, $user_id, $image_plugin, $name_smiley,$count)
{
    ...
    if ($query_count && $wpdb->num_rows <> 0)
	{
        foreach ($query_count as $val) 
		{
            ...
            $user_info  = get_userdata($userid);
            $username[] = $user_info->user_login;
			$html .= '<li><a href smiley_id="' . $id . '" user_id="' . $user_id . '" class="ai_bp_reactions expand" original-title="' . esc_attr($name_smiley) .'" selected="selected"><img src="' . esc_url($image_plugin) . '" class="smiley_img ai_'.$count.'" smiley_id="' . $id . '"></a></li>';
        }
	 }
    return $html;
}
